...
<#
.SYNOPSIS
    Retrieves Azure VM pricing (PAYGO, Reserved Instances, and Savings Plans) for a list of
    SKUs in a region, applies an Azure Consumption Discount (ACD), and exports a CSV or
    Markdown comparison table.

.DESCRIPTION
    For each requested VM SKU the script calls the public Azure Retail Prices API and gathers:
...
.PARAMETER Skus
    One or more Azure VM ARM SKU names (e.g. 'Standard_D2s_v5', 'Standard_E4s_v5').
...
.PARAMETER OS
    Operating system pricing to retrieve: 'Linux' (default) or 'Windows'. Reserved Instance
    prices are compute-only and identical for both; PAYGO and Savings Plan prices differ.
...
.PARAMETER OutputFileName
    ...
    'VmPricing_{region}_{yyyyMMdd}'. The correct extension is appended automatically.

.EXAMPLE
    .\Get-AzureVmPricing.ps1 -Skus 'Standard_D2s_v5','Standard_E4s_v5' -Region eastus -ACD 15

    Prices two SKUs in East US with a 15% ACD and writes a CSV report.

.EXAMPLE
    .\Get-AzureVmPricing.ps1 -Skus Standard_D2s_v5 -Region westeurope -ACD 20 -OutputFormat Markdown

    Produces a Markdown report with discount percentages and best-value highlighting.

.EXAMPLE
    'Standard_D2s_v5','Standard_E4s_v5' | .\Get-AzureVmPricing.ps1 -Region eastus -ACD 10

    Pipes SKU names into the script. Objects with an 'ArmSkuName' or 'Name' property
    (e.g. output of Get-AzComputeResourceSku) can also be piped in directly.
...
.NOTES
    ...
    Output file: ./output/VmPricing_{region}_{yyyyMMdd}.csv|.md
#>

[CmdletBinding()]
param (
    [Parameter(Mandatory = $true, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true,
               HelpMessage = "One or more VM ARM SKU names (e.g. 'Standard_D2s_v5'). Accepts pipeline input.")]
    [ValidateNotNullOrEmpty()]
    [Alias('ArmSkuName', 'Name')]
    [string[]]$Skus,
    ...
    [Parameter(Mandatory = $true, HelpMessage = "Azure Consumption Discount as an integer percentage off PAYGO (0-100)")]
    [ValidateRange(0, 100)]
    [int]$ACD,

    [Parameter(Mandatory = $false, HelpMessage = "Output format: CSV (default) or Markdown")]
    ...
    [Parameter(Mandatory = $false, HelpMessage = "Optional base name for the output file (no extension)")]
...
function Get-SkuPricing {
    ...
    $filter = "serviceName eq 'Virtual Machines' and armRegionName eq '$Region' and armSkuName eq '$Sku'"
    $items  = Invoke-RetailPriceQuery -Filter $filter -Currency $Currency

    # Filter to the exact ARM SKU, excluding Spot and Low Priority variants. The friendly
    # skuName field is inconsistent across series (e.g. 'Standard_D2s_v5' vs 'E4s v5'), so we
    # key off the reliable armSkuName and exclude variants by name pattern.
    $items = $items | Where-Object {
        $_.armSkuName -eq $Sku -and $_.skuName -notmatch '(?i)spot|low priority'
    }

    $wantWindows = ($OS -eq 'Windows')

    # PAYGO consumption item for the requested OS. Constrain to genuine Virtual Machines
    # meters: some SKUs also publish look-alike meters for the same armSkuName (e.g.
    # '<series> Cloud Services') that share the name but carry a different price and no
    # savings plan, which would otherwise be picked up incorrectly.
    $paygItem = $items | Where-Object {
        $_.type -eq 'Consumption' -and
        $_.productName -match '^Virtual Machines' -and
        (($_.productName -match 'Windows') -eq $wantWindows)
    ...
    # Reserved Instances are compute-only (OS-agnostic) and carry no Windows variant.
    # retailPrice is the full term cost. Constrain to genuine Virtual Machines meters.
    $resItems = $items | Where-Object {
...
foreach ($sku in $Skus) {
    ...
    try {
        ...
        $pricing = [pscustomobject]@{ Sku = $sku; Payg = $null; Res1 = $null; Res3 = $null; Sav1 = $null; Sav3 = $null }
    }

    if ($null -eq $pricing.Payg) {
        Write-Warning "No PAYGO price found for '$sku' in '$Region'. Verify the SKU name and region."
    }
...
if ([string]::IsNullOrWhiteSpace($OutputFileName)) {
    $OutputFileName = "VmPricing_{0}_{1}" -f $Region, (Get-Date -Format 'yyyyMMdd')
}
...
if ($OutputFormat -eq 'Markdown') {
    ...
    [void]$sb.AppendLine("# Azure VM Pricing Comparison")
    [void]$sb.AppendLine()
    ...
    [void]$sb.AppendLine()
    [void]$sb.AppendLine("_Source: Azure Retail Prices API (api-version $script:RetailApiVersion). Prices exclude any Windows licensing billed separately on Reserved Instances and are subject to change._")
